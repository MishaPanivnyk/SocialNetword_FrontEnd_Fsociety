...
const ChatCloud = ({
  ...
  function formatDateTime(dateTimeString: string, options: string): string {
    ...
    const time = `${formatTwoDigits(date.getHours())}:${formatTwoDigits(
      date.getMinutes()
    )}`;
  ...
  const getChat = async () => {
    try {
      const getHistoryChat = await axios.get(
        `https://social-netword-fsociety-gamma.vercel.app/chat/get_chat_history/${roomId}`
      );
  ...
  useEffect(() => {
    ...
    const CheckNewMessage = async () => {
      try {
        const responseNewMessage = await axios.get<ChatNewMessage>(
          `https://social-netword-fsociety-gamma.vercel.app/chat/check_new_messages/${myProfile.name}`
        );
                ...
                return (
                  <>
                    ...
                    formatDate(message.timestamp) !==
                      formatDate(chatHistory[index - 1].timestamp) ? (
                      <div className="row">
                      ...
                      >
                        <div className="col-12 d-flex align-items-center">
                          <div className={style.message__received}>
                            {<FixBrComponent text={message.text} />}
                          </div>
                          <span className={style.message__staptime}>
