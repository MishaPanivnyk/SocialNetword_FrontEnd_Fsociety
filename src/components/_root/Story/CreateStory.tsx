...
const CreateStory = ({ myProfile }: CreateStoryProps) => {
    ...
    if (file) {
      ...
      video.onloadedmetadata = () => {
        ...
        if (video.duration > MAX_VIDEO_DURATION_SEC) {
          setErrorMessage(
            `Тривалість відео перевищує ${MAX_VIDEO_DURATION_SEC} секунд`
          );
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await axios.post(
        "https://social-netword-fsociety-gamma.vercel.app/stories/stories_create/",
        formData
      );
      setLoading(false);
      handleClose();
      ...
      <Dialog onClose={handleClose} open={open} maxWidth="md">
                    ...
                    {reel.length > 0 ? (
                      <div className="row">
                        <div className="col-12 d-flex justify-content-center">
                          <video
                            className={style.upload__img__post}
                            src={reel}
                            controls
                          />
                        </div>
        ...
        <DialogActions>
          {loading ? (
            ...
            <button
              className={style.add__post__submit}
              type="submit"
