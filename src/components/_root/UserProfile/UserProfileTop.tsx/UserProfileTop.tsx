...
const UserProfileTop: React.FC<UserProfileTopProps> = ({
  userData,
  postOrReels,
  setPostOrReels,
}) => {
  ...
  useEffect(() => {
    if (myFriendList.length !== 0) {
      setIsFollow(
        myFriendList.some((friend) => friend.name === userDataState.name)
      );
    }
            ...
            <div className="col-10 d-flex flex-column justify-content-around">
              <div className={style.follow__btn}>
                <div className="row d-flex justify-content-between">
                  ...
                  <div className="col-4 d-flex justify-content-end">
                    ...
                    {userDataState.name !== myProfile?.name ? (
                      isFollow ? (
                        <button
                          className={style.unfollow__btn}
              ...
              <div className="row">
                <div className="col-12 d-flex justify-content-between">
                  <p className={style.profile__count}>
                    <span>{userData.post_count || 0}</span>Posts
                  </p>
                  <NavLink
                    to={`/${userData.name}/friends`}
                    className={style.profile__count}
                  >
                    <span>{userData.friends_count || 0}</span>Friends
                  </NavLink>
                  <NavLink
                    to={`/${userData.name}/followers`}
                    className={style.profile__count}
                  >
                    <span>{userData.subscribers_count || 0}</span>Followers
                  </NavLink>
                </div>
              </div>
            </div>
          ...
          <div className="row">
            <div className="col-6 d-flex justify-content-center align-items-center">
              <button
                className={
                  postOrReels == "post"
                    ? style.change__post__type__btn__active
                    : style.change__post__type__btn
                }
                onClick={() => {
                  setPostOrReels("post");
                }}
