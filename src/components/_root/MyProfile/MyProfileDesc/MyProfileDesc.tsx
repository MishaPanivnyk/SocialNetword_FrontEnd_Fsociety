...
const MyProfileDesc: React.FC<MyProfileDescProps> = ({
  userData,
  myProfile,
  postOrReels,
}) => {
      ...
      try {
        ...
        if (postOrReels === "post") {
          const response = await axios.get<PostData[]>(
            `https://social-netword-fsociety-gamma.vercel.app/posts/lookPostUser/${userData.name}`
          );
          const sortedPosts = response.data.sort((a, b) => b.id - a.id);
          setUserPosts(sortedPosts);
        } else {
          const response = await axios.get<ReelsData[]>(
            `https://social-netword-fsociety-gamma.vercel.app/reels/reelsUser/${userData.name}`
          );
          const sortedPosts = response.data.sort((a, b) => b.id - a.id);
