    ...
    if (myProfile) {
      const fetchPosts = async () => {
        try {
          let response;
          if (isReelsPage) {
            response = await axios.get<ReelsData[]>(
              `https://social-netword-fsociety-gamma.vercel.app/reels/reelsAll/${myProfile.name}`
            );
            const sortedPosts = response.data.sort((a, b) => b.id - a.id);
            setReels(sortedPosts);
          } else {
            response = await axios.get<PostData[]>(
              `https://social-netword-fsociety-gamma.vercel.app/posts/look/${myProfile.name}`
            );
            const sortedPosts = response.data.sort((a, b) => b.id - a.id);
            ...
            <div className="col-9">
              <Container>
                <div className={style.home__layout}>
                  ...
                  {myProfile ? (
                    isReelsPage ? (
                      <ReelsContainer reels={reels} myProfile={myProfile} />
                    ) : (
                      <PostContainer posts={posts} myProfile={myProfile} />
                    )
                  ) : null}
                </div>
              </Container>
